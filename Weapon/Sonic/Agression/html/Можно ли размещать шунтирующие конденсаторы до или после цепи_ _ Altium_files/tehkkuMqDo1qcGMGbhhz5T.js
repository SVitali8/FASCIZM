var vidyard_player_width_tehkkuMqDo1qcGMGbhhz5T = 640;
var vidyard_player_height_tehkkuMqDo1qcGMGbhhz5T = 360;
var vidyard_html5_tehkkuMqDo1qcGMGbhhz5T = false;
var vidyard_secure_tehkkuMqDo1qcGMGbhhz5T = false;
var vidyard_integration_check_tehkkuMqDo1qcGMGbhhz5T = function() {};
var vidyard_tehkkuMqDo1qcGMGbhhz5T_params = {};
var vidyard_tehkkuMqDo1qcGMGbhhz5T_raw_params = '';
...
(function() {
  ...
  var embed_script = document.getElementById('vidyard_embed_code_tehkkuMqDo1qcGMGbhhz5T');
  var playerDetails = {"keywords":"","thumbnailUrl":"https://play.vidyard.com/tehkkuMqDo1qcGMGbhhz5T.jpg","@context":"http://schema.org/","@type":"VideoObject","id":"tehkkuMqDo1qcGMGbhhz5T","description":"No description","name":"Powerful PCB Design - Features:ADSCvid","transcript":"In a world of multi-tasking and multi-board designs,\nhaving a complex design doesn't mean you\nneed to deal with a complicated workflow\nand difficult to implement design tools.\nThe Altium Designer platform, coupled with\n64-bit architecture and multi-threading,\nprovides the only easy-to-use PCB design solution that\nincludes everything you need\nto create complex boards - fast and with ease.\nAltium Designer leverages 64-bit architecture\nto provide optimal performance,\nNATIVE 3D model rendering, and design automation to\nsimplify your daily design workflow.\nMulti-threaded design automation\nfeatures for polygon repouring,\nOnline DRC, and physical connectivity analysis\ncan leverage your full system memory,\nso you can let your RAM &amp; GPU do the heavy\nlifting while you save on design time.\nYou can SEE great performance and spend your saved time\ntackling important design challenges like routing.\nMaximize your design scope with large,\nmulti-board projects and\nhigh density topologies without sacrificing\nperformance on daily design activities.\nThen, multi-threaded batch output generation lets your\nsystem simultaneously create the\nfiles you need for manufacturing.\nWhen you have a solid software architecture\nassisting your daily design workflow,\nyou can design and release large, complex boards FAST.\n","uploadDate":"2017-12-06T00:22:20.000Z","duration":"T1M35S","embedUrl":"https://play.vidyard.com/tehkkuMqDo1qcGMGbhhz5T","height":360,"width":640};
  ...
  function check_for_duplicate_embeds() {
    ...
    for (var i = 0; i < all_scripts.length; ++i) {
      if (all_scripts[i].getAttribute('id') === "vidyard_embed_code_tehkkuMqDo1qcGMGbhhz5T") {
    ...
    if (dupe_scripts.length >= 1) {
      ...
      if (typeof document.querySelectorAll === 'function') {
        ...
        for (var i = 0; i < wrappers.length; ++i) {
          if (wrappers[i].getAttributeNode('onclick').value === "fn_vidyard_tehkkuMqDo1qcGMGbhhz5T();") {
  ...
  if (!embed_script || !embed_script.src) {
    ...
    for (var i in scripts) {
      ...
      if (script.src && /^.+(vidyard\.com).*\/tehkkuMqDo1qcGMGbhhz5T.*/.test(script.src)) {
        embed_script = script;
        break;
      }
    }
  ...
  if (embed_script && embed_script.src) {
    ...
    vidyard_tehkkuMqDo1qcGMGbhhz5T_raw_params = rawQueryParams;
    ...
    embed_script.src.replace(/(?:[\?&])([^&=]*)=?([^&#]*)/g, function ($0, $1, $2) {
      if ($1) {
        switch ($1) {
          ...
          case 'vylegacy':
            vidyard_tehkkuMqDo1qcGMGbhhz5T_params[$1] = (parseInt($2, 10) !== 0);
            break;
          case 'chapter':
            vidyard_tehkkuMqDo1qcGMGbhhz5T_params[$1] = parseInt($2, 10);
            break;
          case 'width':
          ...
          case 'height':
            ...
            break;
          case 'X-VY-Signature':
            vidyard_secure_tehkkuMqDo1qcGMGbhhz5T = true;
          default:
            vidyard_tehkkuMqDo1qcGMGbhhz5T_params[$1] = $2;
            break;
        }
      }
    });
  ...
  if (!vidyard_tehkkuMqDo1qcGMGbhhz5T_params.type || vidyard_tehkkuMqDo1qcGMGbhhz5T_params.type !== 'lightbox') {
    vidyard_tehkkuMqDo1qcGMGbhhz5T_params.type = 'inline';
  }

  // Determine Player Settings which effect embed
  var multi_chapter = false && !(typeof vidyard_tehkkuMqDo1qcGMGbhhz5T_params.hide_html5_playlist !== 'undefined' && vidyard_tehkkuMqDo1qcGMGbhhz5T_params.hide_html5_playlist);
  var playlist_always_open = false;
  if (typeof vidyard_tehkkuMqDo1qcGMGbhhz5T_params.playlist_always_open !== 'undefined') {
    playlist_always_open = vidyard_tehkkuMqDo1qcGMGbhhz5T_params.playlist_always_open;
  }
  ...
  if (query_player_width && query_player_width > 0 && query_player_height && query_player_height > 0) {
    vidyard_player_width_tehkkuMqDo1qcGMGbhhz5T = query_player_width;
    vidyard_player_height_tehkkuMqDo1qcGMGbhhz5T = query_player_height;
  } else if (query_player_width && query_player_width > 0) {
    query_player_height = (query_player_width * vidyard_player_height_tehkkuMqDo1qcGMGbhhz5T) / vidyard_player_width_tehkkuMqDo1qcGMGbhhz5T;
    vidyard_player_width_tehkkuMqDo1qcGMGbhhz5T = query_player_width;
    vidyard_player_height_tehkkuMqDo1qcGMGbhhz5T = query_player_height;
  } else if (query_player_height && query_player_height > 0) {
    query_player_width = (query_player_height * vidyard_player_width_tehkkuMqDo1qcGMGbhhz5T) / vidyard_player_height_tehkkuMqDo1qcGMGbhhz5T;
    vidyard_player_width_tehkkuMqDo1qcGMGbhhz5T = query_player_width;
    vidyard_player_height_tehkkuMqDo1qcGMGbhhz5T = query_player_height;
  }

  var iframe_container = document.getElementById("vidyard_tehkkuMqDo1qcGMGbhhz5T");
  ...
  if (!iframe_container && vidyard_tehkkuMqDo1qcGMGbhhz5T_params.type === 'inline') {
    iframe_container = document.createElement("span");
    iframe_container.id = 'vidyard_tehkkuMqDo1qcGMGbhhz5T';
  ...
  if (vidyard_secure_tehkkuMqDo1qcGMGbhhz5T === false) {
    // Get cookie data/ids from various MAPs/CRMs
    var pardot_id = fn_getCookie("pardot");
    if (pardot_id) {
      vidyard_tehkkuMqDo1qcGMGbhhz5T_params.pardot_id = pardot_id;
    }

    var hubspot_id = fn_getCookie("hubspotutk");
    if (hubspot_id) {
      vidyard_tehkkuMqDo1qcGMGbhhz5T_params.hubspot_id = hubspot_id;
    }
    ...
    if (marketo_id && munchkin_id && marketo_id.toLowerCase().indexOf(munchkin_id) > -1) {
      vidyard_tehkkuMqDo1qcGMGbhhz5T_params.marketo_id = encodeURIComponent(marketo_id);
    }

    var dreamforce_id = fn_getCookie("vy_dreamforce");
    if (dreamforce_id) {
      vidyard_tehkkuMqDo1qcGMGbhhz5T_params.dreamforce_id = encodeURIComponent(dreamforce_id);
    }
    ...
    if (vycustom_id !== "") {
      vidyard_tehkkuMqDo1qcGMGbhhz5T_params.custom_id = vycustom_id;
    }

    // Gets the vyemail query string param, which connects views to an email entered
    var vyemail = get_parameter_by_name('vyemail');
    if (vyemail !== "") {
      vidyard_tehkkuMqDo1qcGMGbhhz5T_params.email = vyemail;
    }

    var vyac = get_parameter_by_name('vyac');
    var vyplayer = get_parameter_by_name('vyplayer');
    if (vyplayer === 'tehkkuMqDo1qcGMGbhhz5T') {
      vidyard_tehkkuMqDo1qcGMGbhhz5T_params.access_code = vyac;
    }
  }
  ...
  if (navigator.userAgent.toLowerCase().match(/ip(ad|od|hone)|android/)) {
    vidyard_html5_tehkkuMqDo1qcGMGbhhz5T = true;
    vidyard_tehkkuMqDo1qcGMGbhhz5T_params.disable_popouts = true;
  }
  ...
  function create_embed() {
    ...
    var integration_tracking_check = function() {
      if (playerReady) {
        ...
        window.addEventListener('message', function callback(e) {
          ...
          if (data.event === "ready" && data.uuid === "tehkkuMqDo1qcGMGbhhz5T") {
    ...
    var integration_tracking_check_original = function () {
      ...
      if (!referringUrlSent && vidyard_secure_tehkkuMqDo1qcGMGbhhz5T === true && vidyard_tehkkuMqDo1qcGMGbhhz5T_params.referring_url) {
        var playerFrame = Vidyard.Helpers.getPlayerIFrame('tehkkuMqDo1qcGMGbhhz5T');
        var message = {
          'event': 'parentReferer',
          'uuid': 'tehkkuMqDo1qcGMGbhhz5T',
          'data': {
            'value': vidyard_tehkkuMqDo1qcGMGbhhz5T_params.referring_url
          }
        }
      ...
      if (!vidyard_tehkkuMqDo1qcGMGbhhz5T_params.pardot_id) {
        var pardot_id = fn_getCookie("pardot");
        if (pardot_id) {
          vidyard_tehkkuMqDo1qcGMGbhhz5T_params.pardot_id = pardot_id;
      ...
      if (!vidyard_tehkkuMqDo1qcGMGbhhz5T_params.hubspot_id) {
        var hubspot_id = fn_getCookie("hubspotutk");
        if (hubspot_id) {
          vidyard_tehkkuMqDo1qcGMGbhhz5T_params.hubspot_id = hubspot_id;
      ...
      if (!vidyard_tehkkuMqDo1qcGMGbhhz5T_params.dreamforce_id) {
        var dreamforce_id = fn_getCookie("vy_dreamforce");
        if (dreamforce_id) {
          dreamforce_id = encodeURIComponent(dreamforce_id);
          vidyard_tehkkuMqDo1qcGMGbhhz5T_params.dreamforce_id = dreamforce_id;
          messages.push({'data': {'type': 'dreamforce', 'value': dreamforce_id}});
        }
      }
      if (!vidyard_tehkkuMqDo1qcGMGbhhz5T_params.marketo_id) {
        var marketo_id = fn_getCookie("_mkto_trk");
        var munchkin_id = '';
        ...
        if (marketo_id && munchkin_id && marketo_id.toLowerCase().indexOf(munchkin_id) > -1) {
          ...
          vidyard_tehkkuMqDo1qcGMGbhhz5T_params.marketo_id = marketo_id;
      ...
      if (vidyard_tehkkuMqDo1qcGMGbhhz5T_params.type === "inline" || vidyard_secure_tehkkuMqDo1qcGMGbhhz5T === true) {
        for(var i in messages) {
          messages[i].event = 'associateVisitor';
          messages[i].uuid = 'tehkkuMqDo1qcGMGbhhz5T';
          Vidyard.Helpers.getPlayerIFrame('tehkkuMqDo1qcGMGbhhz5T').contentWindow.postMessage(JSON.stringify(messages[i]), window.location.protocol + '//play.vidyard.com');
        }
      }
    ...
    vidyard_integration_check_tehkkuMqDo1qcGMGbhhz5T = integration_tracking_check;

    // Display the inline or lightbox
    if (vidyard_tehkkuMqDo1qcGMGbhhz5T_params.type === "inline") {
      // Load the iframe
      iframe_container.innerHTML = fn_vidyard_build_iframe_tehkkuMqDo1qcGMGbhhz5T(false, vidyard_player_width_tehkkuMqDo1qcGMGbhhz5T, vidyard_player_height_tehkkuMqDo1qcGMGbhhz5T);

      Vidyard.Helpers.addListener(
        'load',
        'onload',
        function() {
          fn_vidyard_iframe_after_load_tehkkuMqDo1qcGMGbhhz5T();
          integration_tracking_check();
        },
        Vidyard.Helpers.getPlayerIFrame('tehkkuMqDo1qcGMGbhhz5T')
      );

    } else if (vidyard_tehkkuMqDo1qcGMGbhhz5T_params.type === "lightbox") {
      ...
      if (document.readyState === 'complete') {
        if (!vidyard_secure_tehkkuMqDo1qcGMGbhhz5T) {
          integration_tracking_check();
        }
  ...
  Vidyard._players['tehkkuMqDo1qcGMGbhhz5T'] = '';
}());
...
var vyContactCallback = function(data) {
  ...
  vidyard_tehkkuMqDo1qcGMGbhhz5T_params.marketo_lead = marketoData;
  ...
  // or tried again after IFrame loads in fn_vidyard_iframe_after_load_tehkkuMqDo1qcGMGbhhz5T
  playerIframe = Vidyard.Helpers.getPlayerIFrame('tehkkuMqDo1qcGMGbhhz5T');
  if ( playerIframe ) {
    message = {'data': {'type': 'marketo', 'value': marketoData}};
    message.event = 'identifyVisitor';
    message.uuid = 'tehkkuMqDo1qcGMGbhhz5T';
...
(function () {
  function async_load() {
    var contactId = vidyard_tehkkuMqDo1qcGMGbhhz5T_params.marketo_id;
    if (!contactId) {
    ...
    var url = window.location.protocol + '//play.vidyard.com'
        + '/v3/tehkkuMqDo1qcGMGbhhz5T/contact/' + encodeURIComponent(contactId) + '/marketoContact.js';
...
function fn_vidyard_build_iframe_tehkkuMqDo1qcGMGbhhz5T(lightbox, iframe_width, iframe_height) {
  if (lightbox) {
    vidyard_tehkkuMqDo1qcGMGbhhz5T_params.autoplay = true;
  }

  var referring_url = encodeURIComponent(document.referrer);
  if (referring_url) {
    vidyard_tehkkuMqDo1qcGMGbhhz5T_params.referring_url = referring_url;
  }
  ...
  if (document.documentMode === 7) {
    ...
    var ie7RepaintIframe = function() {
      var iframe = Vidyard.Helpers.getPlayerIFrame('tehkkuMqDo1qcGMGbhhz5T');
  ...
  if (vidyard_secure_tehkkuMqDo1qcGMGbhhz5T === true) {
    query_params_string = vidyard_tehkkuMqDo1qcGMGbhhz5T_raw_params;
  } else {
    for (var param in vidyard_tehkkuMqDo1qcGMGbhhz5T_params) {
      var param_value = vidyard_tehkkuMqDo1qcGMGbhhz5T_params[param];
      if (param_value === true) {
        param_value = '1';
      }
...
function fn_vidyard_iframe_after_load_tehkkuMqDo1qcGMGbhhz5T() {
  ...
  if (_theframe) {
    ...
    if (vidyard_secure_tehkkuMqDo1qcGMGbhhz5T === true && vidyard_tehkkuMqDo1qcGMGbhhz5T_params.type === 'lightbox') {
      vidyard_integration_check_tehkkuMqDo1qcGMGbhhz5T();
    }
    ...
    if (vidyard_tehkkuMqDo1qcGMGbhhz5T_params.marketo_lead) {
      var message = {'data': {'type': 'marketo', 'value': vidyard_tehkkuMqDo1qcGMGbhhz5T_params.marketo_lead }};
      message.event = 'identifyVisitor';
      message.uuid = 'tehkkuMqDo1qcGMGbhhz5T';
      Vidyard.Helpers.getPlayerIFrame('tehkkuMqDo1qcGMGbhhz5T')
...
function fn_vidyard_tehkkuMqDo1qcGMGbhhz5T() {
  var vidyard = {};
    ...
    return {
      show:function(initial_iframe_settings) {
        var showCore = function () {
          ...
          var width_height = this.lock_to_aspect_ratio(iframe_settings.width, iframe_settings.height, (319 + (vidyard_player_width_tehkkuMqDo1qcGMGbhhz5T / 5)));
      ...
      fill_iframe: function(settings_html_url, has_element, use_post_request, should_animate, width, height) {
        if(has_element) {
          if(iframe_settings.image) {
            ...
            this.set_tinner_size(fn_vidyard_build_iframe_tehkkuMqDo1qcGMGbhhz5T(true, width, height), should_animate, width, height);
          } else {
      ...
      resize: function() {
        ...
        var width_height = v_box.lock_to_aspect_ratio(
            vidyard_player_width_tehkkuMqDo1qcGMGbhhz5T,
            vidyard_player_height_tehkkuMqDo1qcGMGbhhz5T,
            319 + (vidyard_player_width_tehkkuMqDo1qcGMGbhhz5T / 5)
        );

        iframe_settings.width = width_height[0];
        iframe_settings.height = width_height[1];

        var vidyard_iframe = Vidyard.Helpers.getPlayerIFrame('tehkkuMqDo1qcGMGbhhz5T');
        var vidyard_span = document.getElementById('vidyard_span_tehkkuMqDo1qcGMGbhhz5T');
      ...
      fade_element: function(element_to_fade, fading_in, target_alpha) {
        ...
        if (old_opacity == target_alpha) {
          ...
          if (fading_in == -1) {
            ...
            Vidyard.Helpers.getPlayerIFrame('tehkkuMqDo1qcGMGbhhz5T').src = '';
  ...
  vidyard.box.show({
    iframe: '#', width: vidyard_player_width_tehkkuMqDo1qcGMGbhhz5T,
    height: vidyard_player_height_tehkkuMqDo1qcGMGbhhz5T,
    animate: !vidyard_html5_tehkkuMqDo1qcGMGbhhz5T,
    fixed: true
  });
}
