...
$(function () {
  ...
  if (!isOpenSourceBlog) {
    ...
    var newHighlightReplace = function newHighlightReplace() {
      if (!newHighlightDataMap || newHighlightDataMap.size === 0) {
        return;
      }
      console.log('newHighlightDataMap', newHighlightDataMap);
      var isChinese = /[\u4e00-\u9fa5]/i;
      var keywordList = Array.from(newHighlightDataMap.keys());
      var regKeywordList = keywordList.map(function (val) {
      ...
      if (regKeywordList.length === 0) {
        return;
      }
      var regKeyWord = new RegExp(regKeywordList.join('|'), 'ig');
      var fullSpm = window.csdn.report.getFullSpm('3001.11407');
      var domList = $("#content_views").children("p,h1,h2,h3,h4,h5,h6");

      domList.each(function (domIndex, domNode) {
        var htmlBody = '';
        var childNodes = domNode.childNodes;
        for (var idx = 0; idx < childNodes.length; idx++) {
          var node = childNodes[idx];
          if (node.nodeType === 3 || node.nodeName === '#text') {
            var curText = node.textContent.replace(/</ig, '&lt;');
            var hasHighlight = false;
            curText = curText.replace(regKeyWord, function (val) {
              var keyword = val.toLowerCase();
              var item = newHighlightDataMap.get(keyword);
              if (!item) {
                return val;
              }
              if (item.cache >= 1 || preHlKeyword === keyword || hasHighlight) {
                return val;
              }
              hasHighlight = true;
              item.cache = (item.cache || 0) + 1;
              preHlKeyword = keyword;
              var reportDataStr = JSON.stringify({
                spm: '3001.11407',
                dest: item.link || '',
                extra: item.reportData || {}
              });
              var escapedReportData = reportDataStr.replace(/"/g, '&quot;');
              var displayWord = val || item.word;
              var styleTxtColor = item.color ? 'style="color: ' + item.color + '"' : '';
              var imgHtml = item.icon ? '<i style="background-image: url(' + item.icon + ');" alt="' + displayWord + '" class="words-blog-icon"></i>' : '';
              return '<span ' + styleTxtColor + ' class="new-words-blog" data-href="' + (item.link || '') + '" data-report-view="' + escapedReportData + '" data-report-click="' + escapedReportData + '" data-report-query="spm=' + fullSpm + '" data-tit="' + displayWord + '">\n              ' + displayWord + ' ' + imgHtml + '\n              </span>';
            });
            htmlBody += curText;
          } else if (node.nodeType === 1 && node.outerHTML) {
            htmlBody += node.outerHTML;
          }
        }
        domNode.innerHTML = htmlBody;
      });
      $(document).on('click', '.new-words-blog', function () {
        var href = $(this).attr('data-href');
        var hrefQuery = $(this).attr('data-report-query');
        if (href) {
          var curHref = href.includes('?') ? href + '&' + hrefQuery : href + '?' + hrefQuery;
          goUrl(curHref);
        }
      });
    };
    ...
    var combineHighlightWords = function combineHighlightWords(dataList) {
      // 如果 gitcode 请求还在进行中，或者已经初始化过，则直接返回
      newHighlightRequestData = dataList;
      if (gitcodePending || initedNewHighlight) {
        return;
      }

      // 标记为已初始化，避免重复处理
      initedNewHighlight = true;

      // 一次遍历：去重、填充 newHighlightDataMap、添加 cache、执行高亮替换
      for (var i = 0, len = dataList.length; i < len; i++) {
        var item = dataList[i];
        var keywordLower = (item.word || '').toLowerCase();
        if (!keywordLower || gitcodeMap.has(keywordLower)) {
          continue;
        }
        if (typeof item.cache === 'undefined') {
          item.cache = 0;
        }
        newHighlightDataMap.set(keywordLower, item);
      }
      newHighlightReplace();
    };
          ...
          curText = curText.replace(regKeyWord, function (val) {
            ...
            if (type === 'gitcode') {
              reportData = JSON.stringify({
                ...
                extra: JSON.stringify({
                  words: val,
                  isNewGitCodeAB: isNewGitCodeAB ? 1 : 0
                })
              });
            }
            if (cache[keyword] >= 1 || preHlKeyword === keyword || hasHighlight) {
              ...
              preHlKeyword = keyword;
              if (isInApp()) {
                return '<a data-href="' + searchHref + '" class="app-hl hl ' + getClass(keyword) + '" data-track-view=\'' + reportData + '\' data-report-click="' + reportData.replace(/"/g, '&quot;') + '">' + val + '</a>';
              } else {
                if (type === 'gitcode') {
                  ...
                  return '<a href="' + searchHref + '" target="_blank" class="hl ' + getClass(keyword) + '" data-report-view=\'' + reportData + '\' data-report-click="' + reportData.replace(/"/g, '&quot;') + '" data-tit="' + val + '" data-pretit="' + preHlKeyword + '">' + val + '</a>';
                }
              }
            }
          });
        ...
        if (item.nodeName === '#text') {
          ...
          curText = curText.replace(regKeyWord, function (val) {
            ...
            if (cache[keyword] >= 1 || preHlKeyword === keyword || hasHighlight) {
              ...
              cache[keyword] = cache[keyword] ? cache[keyword] + 1 : 1;
              preHlKeyword = keyword;
              return '<span class="doubao-hl hl " data-report-view="' + reportData.replace(/"/g, '&quot;') + '" data-report-click="' + reportData.replace(/"/g, '&quot;') + '" data-tit="' + val + '" data-pretit="' + preHlKeyword + '">' + val + '</span>';
            }
          });
          htmlBody += curText;
        } else {
    ...
    var gitcodePending = true;
    var initedNewHighlight = false;
    var newHighlightDataMap = new Map();
    var newHighlightRequestData = null;

    if (!isApp) {
      if (Object.hasOwnProperty.call(window, 'cJTemplate') && (cJTemplate == 1 || cJTemplate == 2 || cJTemplate == 0) || Object.hasOwnProperty.call(window, 'runProjectFlag') && runProjectFlag) {
      ...
      if (isNewGitCodeAB) {
        ...
        $.ajax({
          ...
          complete: function complete() {
            eduKey = getKeyword(privateEduData) || [];
            wordsHightInit();
            combineHighlightWords(newHighlightRequestData);
            gitcodePending = false;
          }
        });
      } else {
        gitcodekey = getKeyword(privateData) || [];
    ...
    if (!isGitCodeBlog) {
      var csdnhttpScript = document.createElement('script');
      csdnhttpScript.src = 'https://g.csdnimg.cn/csdn-http/1.0.2/csdn-http.js';
      csdnhttpScript.type = 'text/javascript';
      csdnhttpScript.onload = function () {
        http = window.csdn.httpGateway({
          appKey: '203803574', // 必传
          appSecret: '9znpamsyl2c7cdrr9sas0le9vbc3r6ba', // 必传
          env: 'pre'
        });
        http.post('https://bizapi.csdn.net/api/v1/highlight/match', {
          content: content,
          imei: getCookie('uuid_tt_dd')
        }, {
          headers: { 'Content-Type': 'application/json;charset=UTF-8' },
          credentials: 'include'
        }).then(function (res) {
          if (res.code == 200 && res.data && res.data.length > 0) {
            combineHighlightWords(res.data);
          }
        }).catch(function (err) {
          // console.log('match error', err);
        }).finally(function () {
          // console.log('match finally');
        });
      };
      csdnhttpScript.onerror = function () {
        console.error('csdn-http.js 插件加载失败');
      };
      document.head.appendChild(csdnhttpScript);
    }
